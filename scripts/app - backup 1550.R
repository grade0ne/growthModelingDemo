...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(
      width = 6,
      br(),
      withMathJax(uiOutput("math_explainer")),
...
server <- function(input, output, session) {
  y0_fixed <- 5
  times <- reactive(seq(0, 12, length.out = 25))
  ...
  output$true_equation <- renderUI({
    HTML("$$ \\frac{dN}{dt} = (r - \\alpha N)N $$")
  })
  
  output$math_explainer <- renderUI({
    HTML(paste0(
      "<p><b>Model being fit:</b></p>",
      "<div class='equation-wrap'>$$ \\frac{1}{N}\\frac{dN}{dt} = r - \\alpha N $$</div>",
      "<p>The app generates noisy data from the true parameter values at left, then tries to recover <b>r</b> and <b>alpha</b> by minimizing the <b>sum of squared errors (SSE)</b> between observed data and model predictions.</p>"
    ))
  })
  ...
  output$curve_plot <- renderPlot({
    ...
    plot(
      ...
      main = "The optimizer keeps changing the curve to reduce mismatch",
      pch = 16,
      cex = 1,
      xaxs = "i",
      yaxs = "i"
    ...
    subtitle_text <- paste0(
      "Displayed trial ", state$frame,
      " of ", nrow(path),
      " | current r = ", round(state$r, 4),
      ", current alpha = ", round(state$alpha, 4)
    )
    mtext(subtitle_text, side = 3, line = 0.6, cex = 0.9)
  ...
  output$guess_equation <- renderUI({
    fit <- fit_results()
    
    HTML(paste0(
      "<div class='equation-wrap'>",
      "<div><b>Final fit:</b></div>",
      "<div class='param-grid'>",
      "<div><b>True r:</b> ", round(input$r_true, 4), "</div>",
      "<div><b>Fit r:</b> ", round(fit$best_r, 4), "</div>",
      "<div><b>True alpha:</b> ", round(input$alpha_true, 4), "</div>",
      "<div><b>Fit alpha:</b> ", round(fit$best_alpha, 4), "</div>",
      "</div>",
      "<div style='font-size: 15px; margin-top: 10px;'><b>Final SSE:</b> ", round(fit$best_sse, 2), "</div>",
      "</div>"
    ))
  })
